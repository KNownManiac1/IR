...
################################ Live Code ######################################## 
	...
	rows = json.load(urllib.urlopen("http://127.0.0.1:8000/clicks/" + str(user_id) + '/'))['clicks'];
	for row in rows:
	...
	for doc_number in tfidf_scores.keys():
		scores[doc_number] = (0.9*tfidf_scores[doc_number] + 0.1*factor*reco_sys_scores.data[0].data[doc_number]).item()
	print scores
	print tfidf_scores
